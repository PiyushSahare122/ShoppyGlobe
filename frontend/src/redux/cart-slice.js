...
export const addToCart = createAsyncThunk(
  ...
  async ({ userId, productId, quantity }) => {
    const response = await axios.post(
      "https://shoppy-globe-43nq.vercel.app/api/shop/cart/add", 
      {
        userId,
        productId,
        quantity,
...
export const fetchCartItems = createAsyncThunk(
  ...
  async (userId) => {
    const response = await axios.get(
      `https://shoppy-globe-43nq.vercel.app/api/shop/cart/get/${userId}` 
    );
...
export const deleteCartItem = createAsyncThunk(
  ...
  async ({ userId, productId }) => {
    const response = await axios.delete(
      `https://shoppy-globe-43nq.vercel.app/api/shop/cart/${userId}/${productId}` 
    );
...
export const updateCartQuantity = createAsyncThunk(
  ...
  async ({ userId, productId, quantity }) => {
    const response = await axios.put(
      "https://shoppy-globe-43nq.vercel.app/api/shop/cart/update-cart", 
      {
        userId,
        productId,
        quantity,
