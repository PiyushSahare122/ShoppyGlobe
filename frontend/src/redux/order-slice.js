...
export const createNewOrder = createAsyncThunk(
  ...
  async (orderData) => {
    const response = await axios.post(
      "https://shoppy-globe-43nq.vercel.app/api/shop/order/create",
      orderData 
    );
...
export const capturePayment = createAsyncThunk(
  ...
  async ({ paymentId, payerId, orderId }) => {
    const response = await axios.post(
      "https://shoppy-globe-43nq.vercel.app/api/shop/order/capture", 
      {
        paymentId,
        payerId,
        orderId,
...
export const getAllOrdersByUserId = createAsyncThunk(
  ...
  async (userId) => {
    const response = await axios.get(
      `https://shoppy-globe-43nq.vercel.app/api/shop/order/list/${userId}` 
    );
...
export const getOrderDetails = createAsyncThunk(
  ...
  async (id) => {
    const response = await axios.get(
      `https://shoppy-globe-43nq.vercel.app/api/shop/order/details/${id}` 
    );
